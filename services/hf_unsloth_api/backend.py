...
class HFUnslothTrainerBackend(BaseTrainerBackend):
    ...
    def __init__(self, job_runner: Any | None = None) -> None:
        ...
        self.runtime_name = os.getenv("TRAINER_RUNTIME_NAME", "hf-unsloth-runtime")
    ...
    def build_job_spec(self, request: TrainingRequest) -> Dict[str, Any]:
        ...
        if unsloth_opts:
            ...
            env_value = json.dumps(payload)
            if isinstance(trainer_env, dict):
                trainer_env = {**trainer_env, "UNSLOTH_OPTIONS_JSON": env_value}
            elif isinstance(trainer_env, list):
                trainer_env = trainer_env + [{"name": "UNSLOTH_OPTIONS_JSON", "value": env_value}]
            elif trainer_env is None:
                trainer_env = {"UNSLOTH_OPTIONS_JSON": env_value}
